...
#####################################################      
    ...
    def additem(assignment, P, value):  # used to add The proposition to the assignment dictionary
        copyass = copy.deepcopy(assignment)         
        copyass.update({P : value})        
        return copyass   
#####################################################
...
#####################################################
#####################################################
        #DPLL RECURSION FUNCTION
#####################################################
#####################################################  
    def DPLL(clauses, symbols, assignment):            
        global answer
        if instance.is_satisfied(assignment): # this is our main condition for the recursion. if this is true we exit           
            print(assignment) 
            answer = assignment
            return True  
        elif instance.is_notsatisfied(assignment):   # returns confirmation if evaluates to false.
            return False                  
...
#####################################################
#####################################################        \\|/ 
        #ENTRY POINT OF DPLL RECURSION FUNCTION +++++++++++++++-- IGNITED
#####################################################        /|\|
#####################################################      
    global instancetracker         
    instancetracker += 1 # to track which instance is currently being solved
    starttime = time.clock()
    DPLL(instance.clauses, instance.symbols, assignment) 
    endtime = time.clock() 
    print('{} {} {}'.format('instance#', instancetracker, (endtime - starttime)))
...
with open("bonus_instances_inferred.txt", "w") as output_file:
    starttime = time.clock()  
